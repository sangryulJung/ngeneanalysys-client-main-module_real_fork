...
public class WordCreator {
    ...
    private final String[] essentialGenes = {"CSF3R", "CALR","IDH1", "KIT", "TET2", "IDH2", "RB1", "IKZF1", "NPM1", "NRAS", "SRSF2",
        "TP53", "ASXL1", "CEBPA", "DNMT3A", "RUNX1", "FLT3", "SETBP1", "JAK2", "MPL", "SF3B1"};

    public void updateEmbeddedDoc(String path) throws IOException {
    ...
    public void updateEmbeddedDoc(InputStream file) throws IOException {
        doc = new XWPFDocument(file);
    }

    public void setQCData(List<XWPFTableCell> cells, SampleQC qc) {
        setTableCellText(cells.get(1), qc.getQcThreshold(), 8, false, null);
        setTableCellText(cells.get(2), qc.getQcUnit(), 8, false, null);
        setTableCellText(cells.get(3), qc.getQcValue().toString(), 8, false, null);
    }
    ...
    public void updateWordFile() throws IOException {
    ...
    public void addGene(XWPFTableRow row, VariantCountByGene gene, int startIndex) {
        String fontColor = "000000";
        if(gene != null) {
            if (totalCount(gene) > 0) fontColor = "FF0000";

            setTableCellText(row.getCell(startIndex), gene.getGeneSymbol(), 8, true, fontColor);
            if(Arrays.stream(essentialGenes).anyMatch(str -> str.equalsIgnoreCase(gene.getGeneSymbol()))) {
                addSuperScript(row.getCell(startIndex).getParagraphs().get(0), "*", 8, fontColor, VerticalAlign.SUPERSCRIPT);
            }
            setTableCellText(row.getCell(startIndex + 1), String.valueOf(tier1VariantCount(gene)), 8, false, null);
            setTableCellText(row.getCell(startIndex + 2), String.valueOf(tier2VariantCount(gene)), 8, false, null);
            setTableCellText(row.getCell(startIndex + 3), String.valueOf(tier3VariantCount(gene)), 8, false, null);
            setTableCellText(row.getCell(startIndex + 4), String.valueOf(totalCount(gene)), 8, false, null);
        } else {
            setTableCellText(row.getCell(startIndex), null, 8, false, null);
            setTableCellText(row.getCell(startIndex + 1), null, 8, false, null);
            setTableCellText(row.getCell(startIndex + 2), null, 8, false, null);
            setTableCellText(row.getCell(startIndex + 3), null, 8, false, null);
            setTableCellText(row.getCell(startIndex + 4), null, 8, false, null);
        }
    }
    ...
    public void setQCData(final XWPFTable table) {
        ...
        for(int i = 0 ; i < 5 ; i++)
            setTableCellText(rows.get(i + 1).getCell(3), params.get(item[i]) != null ? params.get(item[i]).toString() : null, 8, false, null);
    ...
    public void setTableCellText(XWPFTableCell cell, String text, int fontSize, boolean bold, String color) {
        XWPFRun run = null;
        ...
        run.setBold(bold);
        if(!ngeneanalysys.util.StringUtils.isEmpty(color)) run.setColor(color);
    }

    public void addVariantRowData(XWPFTableRow row, VariantAndInterpretationEvidence variant) {
        setTableCellText(row.getCell(0), variant.getSnpInDel().getGenomicCoordinate().getGene(), 8, true, null);
        setTableCellText(row.getCell(1), variant.getSnpInDel().getSnpInDelExpression().getTranscript(), 7, false, null);
        setTableCellText(row.getCell(2), variant.getSnpInDel().getSnpInDelExpression().getNtChange(), 7, false, null);
        setTableCellText(row.getCell(3), variant.getSnpInDel().getSnpInDelExpression().getAaChangeConversion(), 7, false, null);
        setTableCellText(row.getCell(4), variant.getSnpInDel().getReadInfo().getAlleleFraction() != null ?
                variant.getSnpInDel().getReadInfo().getAlleleFraction().toString() + "%" : null, 7, false, null);
        setTableCellText(row.getCell(5), variant.getSnpInDel().getReadInfo().getReadDepth() != null ?
                variant.getSnpInDel().getReadInfo().getReadDepth().toString() : "", 7, false, null);
        if(variant.getSnpInDel().getReadInfo().getReadDepth() < 100 ) {
            addSuperScript(row.getCell(5).getParagraphs().get(0), "***", 7, "FF0000", VerticalAlign.SUPERSCRIPT);
        }
        setTableCellText(row.getCell(6), returnTierString(variant.getSnpInDel()), 7, false, null);
        setTableCellText(row.getCell(7), variant.getInterpretationEvidence() != null ?
                variant.getInterpretationEvidence().getClinicalVariantType() : null, 7, false, null);
    ...
    public void setDetectedGeneticVariants(final XWPFTable table, String tier) {
    ...
    public void tableEdit() {
        doc.getTables().stream().forEach(tbl -> {
            ...
            if(tbl != null && caption != null && caption.matches("T1|T2|T3")) {
                setDetectedGeneticVariants(tbl, caption);
    ...
    public void addSuperScript(XWPFParagraph paragraph, String superScript, int fontSize, String fontColor, VerticalAlign align) {
        XWPFRun run = paragraph.createRun();
        if(!ngeneanalysys.util.StringUtils.isEmpty(fontColor)) run.setColor(fontColor);
        run.setFontSize(fontSize);
        run.setSubscript(align);
        run.setText(superScript);
    }
    ...
    public void evidenceRowData(XWPFTableRow row, VariantAndInterpretationEvidence variant, String impacts) {
        ...
        if(impacts.equalsIgnoreCase("Therapeutic")) {
            ...
            setTableCellText(row.getCell(4), null, 8, false, null);
            setTableCellText(row.getCell(5), null, 8, false, null);
        }
    }
    ...
    private static class WordCreatorHelper{
        private WordCreatorHelper(){}
        private static final WordCreator INSTANCE = new WordCreator();
    }

    public static WordCreator getInstance() {
        return WordCreatorHelper.INSTANCE;
    }
}
